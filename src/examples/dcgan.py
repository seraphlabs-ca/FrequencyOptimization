...
try:
    ...
    image_path = os.path.join("..", "data", "generated", "dcgan-%s" % opt.dataset,
                              "images.freq-cutoff_%e__freq-order_%i.%s" % (
                                  opt.freq_cutoff, opt.freq_order,
...
freq_filter = FrequencyFilter(
    ...
    cutoff=opt.freq_cutoff,
    order=opt.freq_order,
)
...
try:
    for epoch in range(opt.niter):
        for i, data in enumerate(dataloader, 0):
            ...
            errD_real = freq_filter.step({"train.errD_real": errD_real}, min_val=1e-6)["train.errD_real"]
            errD_real.backward()
            ...
            errD_fake = freq_filter.step({"train.errD_fake": errD_fake}, min_val=1e-6)["train.errD_fake"]
            errD_fake.backward()
            ...
            errG = freq_filter.step({"train.errG": errG}, min_val=1e-6)["train.errG"]
            errG.backward()
