...
def main():
    ...
    try:
        for epoch in range(args.start_epoch, args.epochs):
            ...
            try:
                common.media.save_all_figs(fdir, im_type="png")
                common.media.save_all_figs(fdir, im_type="html")
                opts.Options(vars(args)).export_as_ini(os.path.join(fdir, "args"))
                opts.Options({
                    "signal": freq_filter.signal_dict,
                    "f_signal": freq_filter.f_signal_dict,
                }).export_as_json(os.path.join(fdir, "results"))
...
def validate(val_loader, model, criterion, freq_filter):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        freq_filter.step({"val.prec": prec.cpu().numpy().tolist()[0]}, min_val=1e-1, max_val=1e1)
